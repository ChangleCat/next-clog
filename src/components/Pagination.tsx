...
export default function Pagination({
  ...
  className = "",
  targetID = null,
  // @example: /pages/pagination/{index}
  URLTemplate,
}: PaginationProps) {
    ...
    <div className={cn("flex justify-between", className)}>
      {isFirstPage ? (
        ...
        <Link
          ...
          className={cn(
            "card-base p-2 hover:text-primary flex items-center justify-center w-24",
            "hover:[&>div]:translate-x-0 hover:[&>div]:opacity-100 hover:[&>div]:w-auto hover:[&>div]:mr-0"
          )}
        >
          <Icon icon="material-symbols:arrow-back-ios-new-rounded" />
          <div className="opacity-0 translate-x-1 transition-all -mr-8 duration-300">
            上页
          </div>
        </Link>
      )}
      <div className="flex gap-2 [&>a]:p-2 [&>a]:w-10 [&>a]:h-10 [&>a]:flex [&>a]:items-center [&>a]:justify-center">
        {!isShowingFirst && (
          <>
            <Link
              href={`${URLTemplate.replace("{index}", "1")}${toScroll}`}
      ...
      {isLastPage ? (
        ...
        >
          <div className="opacity-0 -translate-x-1 transition-all -ml-8 duration-300">
            下页
          </div>
