...
export default function Header() {
  ...
  return (
    <>
      <header
        className={cn(
          "bg-surface-2 w-full p-4 rounded-b-md fixed top-0 z-500 hover:backdrop-blur-[1px] opacity-97 shadow-lg",
      ...
      >
        <div className="flex justify-between items-center max-w-7xl w-full">
          <Link
            href={"/"}
            className="font-bold text-xl block relative outline-none"
            aria-label="主页"
          >
            <div className="relative px-2 py-0.5">
              ...
              <div
                className={cn(
                  "absolute w-full h-full rounded-2xl bg-button top-0 left-0 opacity-0 transition-opacity",
          ...
          <div className="flex gap-2">
            {isMounted && (
              <button
                type="button"
                className={cn(
                  "p-2 rounded-xl hover:bg-button-hover hover:text-text-main-dark transition-colors cursor-pointer active:scale-95"
                )}
      ...
      <Menu
        isMenuFolded={isMenuFolded}
        toggleFunction={handleFoldToggle}
...
function MenuButton({
  ...
  return (
    <button
      ...
      title="Menu"
      className={cn(
        "relative w-8 h-8 rounded-xl cursor-pointer",
...
function Menu({
  ...
  return (
    <div
      className={cn(
        "fixed top-0 left-0 h-svh w-full transition-all z-9999 md:hidden",
        isMenuFolded ? "pointer-events-none" : "backdrop-blur-sm"
      )}
      onMouseDown={(event) => {
    ...
    >
      <div
        id="menu"
        className={cn(
          "h-full w-5/7 absolute right-0 transition-transform bg-surface-1 border border-border",
          "rounded-none",
          isMenuFolded ? "translate-x-full" : ""
        )}
      >
        <nav className="p-4">
          <ul
            className={cn(
              "flex gap-4 flex-col items-stretch",
                ...
                <Link
                  className={cn(
                    ...
                    currentPathname === value.href &&
                      "bg-primary text-text-main-light"
                  )}
                  href={value.href}
